...
1. 在 Supabase Dashboard 進 **SQL Editor**，依序執行 `001_init.sql` 至 `007_persistent_cart.sql`。
   - 會建立包含帳戶刪除申請、尺寸庫存與購物車在內的應用資料表、2 個私有 storage bucket（`person-uploads`、`try-on-results`）、3 件種子商品。
...
- [ ] `001` 至 `007` migrations 已依序在正式 Supabase 專案執行。
