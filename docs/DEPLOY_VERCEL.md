...
1. 在 Supabase Dashboard 進 **SQL Editor**，依序執行 `001_init.sql` 至 `010_inventory_reservations.sql`。
   - 會建立包含帳戶刪除申請、尺寸庫存、購物車、訂單、Mock 付款與庫存保留在內的應用資料表、2 個私有 storage bucket（`person-uploads`、`try-on-results`）、3 件種子商品。
...
9. 建立訂單後，在 Mock Payment Sandbox 分別測試成功、失敗、取消與逾期；成功時訂單應顯示「處理中」，並可在 `/orders` 與詳情頁看到交易編號及 Webhook 紀錄。
   - 建立待付款訂單後，`product_variants.stock_quantity` 不應改變；付款成功才應扣減一次。失敗、取消、逾期後則應維持原庫存並釋放保留量。
...
- [ ] `001` 至 `010` migrations 已依序在正式 Supabase 專案執行。
