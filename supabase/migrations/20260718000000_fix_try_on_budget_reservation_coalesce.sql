-- 修正 PostgreSQL 對 numeric aggregate 與未轉型 integer literal 的 COALESCE 型別解析。
-- 此 migration 只替換既有 quota RPC；不改變 function signature、鎖順序、RLS 或授權。
...
begin
  ...
  -- SUM(numeric) 的空集合預設也必須是 numeric；否則部分 PostgreSQL 版本會把
  -- 未轉型 integer literal 視為不相容的 schema-qualified aggregate fallback 而失敗。
