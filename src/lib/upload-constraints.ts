...
export const MAX_FILE_SIZE_BYTES = 4 * 1024 * 1024; // 4 MiB，低於 Vercel 4.5 MB request 上限
...
export function validateFileMeta(file: { type: string; size: number }): ValidationResult {
  ...
  if (file.size > MAX_FILE_SIZE_BYTES) {
    return {
      ok: false,
      message: `照片超過 4MB。圖片不得超過 4MB，請換一張較小的照片，或先把照片稍微縮小（寬度 ${TARGET_MAX_WIDTH}px 以內即可）再上傳。`,
    };
  }
  if (file.size === 0) {
